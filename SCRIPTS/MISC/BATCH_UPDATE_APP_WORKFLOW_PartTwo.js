...
aa.env.setValue("sendToEmail", "jshear@trustvip.com"); //ca-licensees@metrc.com
...
function mainProcess() {
	...
	for (myCapsXX in capList) {
		...
		if(!matches(recStatus,"Pending Owner Applications","Pending Final Affidavit","Application Fee Due")){
			continue;
		}
		var cap = aa.cap.getCap(capId).getOutput()
		var fileDateField = cap.getCapModel().getFileDate();
		var fileDate = String(fileDateField).split(" ");
		var fileDateArray = fileDate[0].split("-");
		var fileDateFormat = fileDateArray[1] + "/" + fileDateArray[2] + "/" + fileDateArray[0];
		var fileDateObj = new Date(fileDateFormat);
		var batchDateObj = new Date("04/22/2021");
		if (fileDateObj > batchDateObj){
			continue;
		}
		...
		if (workflowResult.getSuccess()){
			...
			for (i in wfObj) {
				fTask = wfObj[i];
				if (String(fTask.getTaskDescription()).equals("Local Verification Review")){
					newWF = true;
				}
			}
		}else{ 
		...
		if (wfResult.getSuccess()) {
			...
			wfUpdate++;
			capArray.push(capId.getCustomID());
		}else {
	...
	logDebug("Active Tasks: " + taskArray);
	logDebug("Processed Records: " + capArray);
}	
