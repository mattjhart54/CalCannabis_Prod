	...
	for (x in r) { 
		if(uploadedDocs[r[x]] == undefined) {	
			...
			if (!docsMissing)  {
				...
				if(appMatch("Licenses/*/*/*")) conditionType = "Renewal Required Documents";
				dr = r[x];
...
if (r.length > 0 && showList && docsMissing) {
	...
	showDebug =true;
	logDebug("An error has occurred in ACA_BEFORE_FOR_REN_DOC_REQS: Load Docs: " + err.message);
	logDebug(err.stack);
	aa.sendMail(sysFromEmail, debugEmail, "", "A JavaScript Error occurred: ACA_BEFORE_FOR_REN_DOC_REQS: Load Docs: " + startDate, "capId: " + capId + br + err.message + br + err.stack + br + currEnv);
}
