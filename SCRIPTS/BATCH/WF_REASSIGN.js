// Update_Workflow_Task_Assignments.js
...
try {
	var userMap = [{"EXISTING":"YULIA.OLIYNYK","NEW":"YULIYA.OLIYNYK"}];
	var ignoreRecordStatuses = ["License Issued", "Provisional License Issued", "Pending Owner Application", "Pending Final Affidavit", "Abandoned", "Disqualified","Withdrawn","Closed","Denied","Voided",null];
	var ignoreWFStatuses = ['License Issued', 'Provisional License Issued','Abandoned', 'Void', 'Withdrawn', 'Completed', 'Closed', 'Denied'];
	var i = 0;
	
	vCapList = aa.cap.getByAppType("Licenses", "Cultivator", null, "Application").getOutput();
//	vCapList= aa.cap.getByAppType("Licenses", "Cultivator", null, "Owner Application").getOutput();
//	vCapList= aa.cap.getByAppType("Licenses", "Cultivator", null, "Amendment").getOutput();
//	vCapList= aa.cap.getByAppType("Licenses", "Cultivator", ""Amendment", null).getOutput();
//	vCapList= aa.cap.getByAppType("Licenses", "Cultivator", null, "Renewal").getOutput();

	i = 0;
	logDebug("Processing " + vCapList.length + " records from Application records");
...
//					logDebug("**Could not locate User ID of assigned user; skipping");
				}
			} else {
