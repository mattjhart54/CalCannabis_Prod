/*------------------------------------------------------------------------------------------------------/
| Program: BATCH_LSA Review Update
...
eval(getScriptText("LSA REVIEW DATA"));
...
try {
	...
	logDebug("ERROR: BATCH_LSA Review Update: " + err.message + " In " + batchJobName + " Line " + err.lineNumber);
...
try{ 
	...
	for (i in LSA) {
		if(!matches(LSA[i]["License_Number"],null,"",undefined)) {
			...
			if (!capId) {
				logDebug("LSA Review row for " + LSA[i]["License_Number"] + " not processed as License record not found");
				rejCnt++;
				continue;
			}
		}
			...
			if (!capId) {
				logDebug("LSA Review row for " + LSA[i]["Application_Number"] + " not processed as Application record not found");
				rejCnt++;
				continue;
			}
		}
		recCnt++;
if (!matches(LSA[i]["CDFW_Review_Status"],null,undefined,"")){
		...
		if (!matches(LSA[i]["APN_Matches_Adjacent_Parcel"],null,undefined,"")){
			editAppSpecific("APN Matches Adjacent Parcel",LSA[i]["APN_Matches_Adjacent_Parcel"]);
		}
